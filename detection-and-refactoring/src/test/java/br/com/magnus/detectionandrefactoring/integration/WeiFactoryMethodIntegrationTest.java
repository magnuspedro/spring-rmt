...
class WeiFactoryMethodIntegrationTest extends BaseIntegrationTest {
    ...
    @Test
    void testFactoryMethodPatternRefactoring() throws IOException {
        var projectId = UUID.randomUUID().toString();

        var javaFiles = Wei.createJavaFilesFactory();
        ...
        var candidateInfo = updatedProject.getCandidatesInformation().stream()
                .filter(candidate -> candidate.getDesignPattern() == DesignPattern.FACTORY_METHOD)
                .findFirst()
                .orElseThrow();
        assertThat(candidateInfo.getId()).isNotBlank();
        assertThat(candidateInfo.getDesignPattern()).isEqualTo(DesignPattern.FACTORY_METHOD);
        assertThat(candidateInfo.getFilesChanged())
                .isNotEmpty()
                .anyMatch(path -> path.endsWith("LoggerFactory.java"));
        ...
        try (var refactoredZip = s3Client.getObject(
            ...
            assertThat(refactoredContent.keySet())
                    .contains("factory/Logger.java")
                    .contains("factory/LoggerFactory.java")
                    .contains("factory/DataBaseLoggerFactory.java")
                    .contains("factory/FileLoggerFactory.java");

            assertThat(refactoredContent.get("factory/LoggerFactory.java"))
                    .contains("public abstract class LoggerFactory")
                    .contains("public abstract Logger createLogger();");
            assertThat(refactoredContent.get("factory/DataBaseLoggerFactory.java"))
                    .contains("extends LoggerFactory")
                    .contains("return logger;");
            assertThat(refactoredContent.get("factory/FileLoggerFactory.java"))
                    .contains("extends LoggerFactory")
                    .contains("return logger;");
        }
